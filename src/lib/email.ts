...
const fromAddress = process.env.RESEND_FROM_EMAIL ?? 'Hälsakoll <noreply@halsakoll.se>';
...
export async function sendCareInviteEmail({
  ...
  return getResend().emails.send({
    from: fromAddress,
    to,
    subject: 'Du har bjudits in till Hälsakoll',
    html: `
      <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 480px; margin: 0 auto; padding: 32px 16px;">
        <h1 style="font-size: 24px; color: #111827; margin-bottom: 16px;">Hälsakoll</h1>
        <p style="font-size: 16px; color: #374151; line-height: 1.6;">
          Hej!
        </p>
        <p style="font-size: 16px; color: #374151; line-height: 1.6;">
          <strong>${inviterEmail}</strong> vill dela sin hälsoöversikt med dig via Hälsakoll.
        ...
        <p style="font-size: 12px; color: #9ca3af;">
          Hälsakoll — din hälsa, tryggt samlad
        </p>
      </div>
    `,
  });
